<template>
    <div v-if="order">
      ...
      <div>
        ...
        <table>
          <tbody>
          <tr>
            <td class="font-bold py-1 px-2">Pedido #</td>
            <td>{{ order.id }}</td>
          </tr>
          <tr>
          ...
          <tr>
            <td class="font-bold py-1 px-2">Total do Pedido</td>
            <td>{{ Intl.NumberFormat('pt-br', {style: 'currency', currency: 'BRL'}).format(order.total_price) }}</td>
          </tr>
          </tbody>
        </table>
      </div>
      ...
      <!--  Customer Details-->
      <div>
        <h2 class="text-xl font-semibold mt-6 pb-2 border-b border-gray-300">Detalhe do Cliente</h2>
        <table>
          <tbody>
          <tr>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          <h2 class="text-xl font-semibold mt-6 pb-2 border-b border-gray-300">Endereço para envio</h2>
      ...
      <!--    Order Items-->
      <div>
        <h2 class="text-xl font-semibold mt-6 pb-2 border-b border-gray-300">Itens do Pedido</h2>
          ...
          <div class="flex flex-col sm:flex-row items-center  gap-4">
            ...
            <div class="flex flex-col justify-between flex-1">
              ...
              <div class="flex justify-between items-center">
                ...
                <span class="text-lg font-semibold"> {{ Intl.NumberFormat('pt-br', {style: 'currency', currency: 'BRL'}).format(item.unit_price) }} </span>
              </div>
            </div>
          </div>
          <!--/ Order Item -->
  ...
  function onStatusChange() {
    axiosClient.post(`/orders/change-status/${order.value.id}/${order.value.status}`)
      .then(({data}) => {
        store.commit('showToast', `O status do pedido foi alterado com sucesso para "${order.value.status}"`)
      })
