<template>
    <div v-if="customer.id" class="animate-fade-in-down">
      <form @submit.prevent="onSubmit">
        <div class="bg-white px-4 pt-5 pb-4">
          ...
          <CustomInput class="mb-2" v-model="customer.first_name" label="Primeiro Nome"/>
          <CustomInput class="mb-2" v-model="customer.last_name" label="Último Nome"/>
          <CustomInput class="mb-2" v-model="customer.email" label="Email"/>
          <CustomInput class="mb-2" v-model="customer.phone" label="Telefone"/>
          <CustomInput type="checkbox" class="mb-2" v-model="customer.status" label="Ativo"/>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                <CustomInput v-model="customer.billingAddress.address1" label="Endereço 1"/>
                <CustomInput v-model="customer.billingAddress.address2" label="Endereço 2"/>
                <CustomInput v-model="customer.billingAddress.city" label="Cidade"/>
                <CustomInput v-model="customer.billingAddress.zipcode" label="CEP"/>
                ...
                <CustomInput v-else type="select" :select-options="billingStateOptions"
                             v-model="customer.billingAddress.state" label="Estado"/>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                <CustomInput v-model="customer.shippingAddress.address1" label="Endereço 1"/>
                <CustomInput v-model="customer.shippingAddress.address2" label="Endereço 2"/>
                <CustomInput v-model="customer.shippingAddress.city" label="Cidade"/>
                <CustomInput v-model="customer.shippingAddress.zipcode" label="CEP"/>
                ...
                <CustomInput v-else type="select" :select-options="shippingStateOptions"
                             v-model="customer.shippingAddress.state" label="Estado"/>
              </div>
            </div>
          </div>
  ...
  onMounted(() => {
    store.dispatch('getCustomer', route.params.id)
      .then(({data}) => {
        title.value = `Editar Cliente: "${data.first_name} ${data.last_name}"`
        customer.value = data
      })
  })
